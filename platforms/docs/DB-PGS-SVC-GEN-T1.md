...
**ShrnutĂ­:** ServisnĂ­ ĂşÄŤet pro PGS na DatabĂˇze. ProstĹ™edĂ­: GenerickĂ©, Tier: T1.
...
- **ZakĂˇzanĂ© znaky:** -$'\/@".;{}()*>~!^#[],:+=<%|
- **ChangeCommand:** ALTER USER %USER% WITH PASSWORD '%NEWPASSWORD%';
- **ReconcileCommand:** ALTER USER %USER% WITH PASSWORD '%NEWPASSWORD%';
- **ConnectionCommand:** Driver={PostgreSQL UNICODE};Server=%ADDRESS%;Database=%DATABASE%;Port=%PORT%; Uid=%USER%;Pwd=%LOGONPASSWORD%;sslmode=require;
- **DLL konektoru:** PMODBC.dll
- **PovolenĂ© safy (regex):** (?!SH-).*_T1$

## Metadata
- INI: `I:\GitHub\platforms\platforms\DB-PGS-SVC-GEN-T1\Policy-DB-PGS-SVC-GEN-T1.ini`
- XML: `I:\GitHub\platforms\platforms\DB-PGS-SVC-GEN-T1\Policy-DB-PGS-SVC-GEN-T1.xml` â€” parsed
...
- DllName: PMODBC.dll
- ExtraInfo::CommandBlackList: delete,drop,exec
- ExtraInfo::CommandForbiddenCharacters: '\/@".{}() |*>~!^#
- ExtraInfo::ConnectionCommand: Driver={PostgreSQL UNICODE};Server=%ADDRESS%;Database=%DATABASE%;Port=%PORT%; Uid=%USER%;Pwd=%LOGONPASSWORD%;sslmode=require;
- ExtraInfo::Debug: Yes
- ExtraInfo::ChangeCommand: ALTER USER %USER% WITH PASSWORD '%NEWPASSWORD%';
- ExtraInfo::ReconcileCommand: ALTER USER %USER% WITH PASSWORD '%NEWPASSWORD%';
- FromHour: -1
- HeadStartInterval: 5
...
- PolicyID: DB-PGS-SVC-GEN-T1
- PolicyName: DB-PGS-SVC-GEN-T1
- PolicyType: Regular
